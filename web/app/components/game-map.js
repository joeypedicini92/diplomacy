...
export default Component.extend({
  ...
  didInsertElement() {
    ...
    svg.addEventListener('load', () => {
      ...
      this.set('svgElement', this.element.getElementsByTagName('object')[0].contentDocument);
      this.set('mypopup', this.element.getElementsByTagName('object')[0].contentDocument.getElementById('tooltip'));
  ...
  applyHoverFunctionToTerritories() {
    let territories = this.svgElement.querySelectorAll('.territory');
  ...
  showPopup(elem) {
    ...
    this.mypopup.textContent = elem.dataset.name.toUpperCase();
    this.svgElement.getElementsByTagName('g')[0].appendChild(this.mypopup);
  },
  ...
  mapDetails: computed('territories', function () {
    this.territories.forEach((t) => {
      let svgEl = this.svgElement.querySelector(`[data-name=${t.id}]`);
      let army = drawUnitOnPath(svgEl, t);
      this.svgElement.getElementsByTagName('g')[0].appendChild(army);
      svgEl.classList.add(t.country.toLowerCase());
    });
  })
